...
export const signin = async (userTable: Table): Promise<Tuple | undefined> => {
  const tableName = Object.keys(userTable)[0];
  const table = userTable[tableName];
  const columnNames = [];
  const columnValues = [];
  for (let col in table) {
    columnNames.push(col);
    columnValues.push(`'${table[col]}'`);
  }
  const res = await executeQuery(
    `select * from ${tableName} where (${columnNames.join(
      ","
    )}) in ((${columnValues.join(",")}))`
  );
